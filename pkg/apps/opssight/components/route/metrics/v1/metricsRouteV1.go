...
func init() {
	store.Register(types.OpsSightMetricsRouteV1, NewOpsSightRoute)
}
...
func NewOpsSightRoute(config *protoform.Config, kubeClient *kubernetes.Clientset, cr interface{}) (types.RouteInterface, error) {
	...
	return &OpsSightRoute{config: config, kubeClient: kubeClient, opsSight: opsSight}, nil
}
...
func (o *OpsSightRoute) GetRoute() (*api.Route, error) {
	...
	if strings.ToUpper(o.opsSight.Spec.Perceptor.Expose) == util.OPENSHIFT {
		return &api.Route{
			Name:               utils.GetResourceName(o.opsSight.Name, util.OpsSightName, fmt.Sprintf("%s-metrics", o.opsSight.Spec.Prometheus.Name)),
			Namespace:          namespace,
			Kind:               "Service",
			ServiceName:        utils.GetResourceName(o.opsSight.Name, util.OpsSightName, o.opsSight.Spec.Prometheus.Name),
			PortName:           fmt.Sprintf("port-%s", o.opsSight.Spec.Prometheus.Name),
			Labels:             map[string]string{"app": "opssight", "name": o.opsSight.Name, "component": "prometheus-metrics"},
